#!/bin/zsh
# ai-news-daily 定时运行脚本（JSON 版）
# 由 launchd 每天 09:00 触发，通过 dim exec 无头模式以 inline script 方式运行 workflow
...
# 用 heredoc 把完整 workflow 脚本传给 dim exec
# dim exec 的无头 agent 会用 workflow 工具以 inline script 方式运行
cat <<'WORKFLOW_EOF' | dim exec 2>&1 >> "$LOGDIR/ai-news-daily.log"
请用 workflow 工具运行以下 inline script。直接调用 workflow 工具，把下面整个脚本作为 script 参数传入，不要修改、不要省略、不要添加任何解释：
...
WORKFLOW_EOF' | dim exec 2>&1 >> "$LOGDIR/ai-news-daily.log"
请用 workflow 工具运行以下 inline script。直接调用 workflow 工具，把下面整个脚本作为 script 参数传入，不要修改、不要省略、不要添加任何解释：
...
export default async function workflow(api) {
  ...
  await api.agent(
    `你是数据工程师。下面是今天(${today})收集的 ${allNews.length} 条 AI 新闻数据(JSON):\n\n${JSON.stringify(allNews, null, 2)}\n\n请:\n1. 按标题相似度做语义去重（保留更完整的一条）\n2. 按重要性和时效性排序，最重要的在前\n3. 用 write 工具把严格合法的 JSON 写入文件: ${outputPath}\n   文件内容格式（不要写任何其它内容到该文件）:\n   {\n     "date": "${today}",\n     "count": 去重后的条数,\n     "news": [\n       { "title": "...", "source": "...", "url": "...", "date": "YYYY-MM-DD", "category": "model|research|business|policy", "summary": "..." }\n     ]\n   }\n   注意: count 必须等于 news 数组长度；category 必须是四个枚举值之一；每条都必须有 title/url/summary。\n4. 完成后回复文件路径和最终 count。`,
    { label: "清洗数据并写入 JSON" }
  )
